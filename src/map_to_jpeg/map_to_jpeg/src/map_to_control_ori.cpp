  ...
  rclcpp::Time stamp = scan_ ? rclcpp::Time(scan_->header.stamp) : this->now();
    ...
    try {
      ...
      tf2::durationFromSec(0.05));
        ...
        RCLCPP_WARN(this->get_logger(), "Could not get laser transform: %s", ex.what());
        return;
    }
    ...
    for (const auto &r : scan_->ranges) {
        if (std::isfinite(r)) {
            // จุดใน laser frame
            tf2::Vector3 pt_laser(r * cos(angle), r * sin(angle), 0.0);

            // แปลงเป็นจุดใน map frame
            tf2::Quaternion q(
            ...
            tf2::Transform tf_map_laser(q,
                tf2::Vector3(
                    laser_to_map.transform.translation.x,
                    laser_to_map.transform.translation.y,
                    laser_to_map.transform.translation.z));

            tf2::Vector3 pt_map = tf_map_laser * pt_laser;

            // แปลงเป็นพิกัด pixel
            int lx = static_cast<int>((pt_map.x() - current_map_->info.origin.position.x) / current_map_->info.resolution);
            int ly = height - static_cast<int>((pt_map.y() - current_map_->info.origin.position.y) / current_map_->info.resolution);

            if (lx >= 0 && lx < width && ly >= 0 && ly < height) {
                cv::circle(map_image, cv::Point(lx, ly), 2, cv::Scalar(0, 255, 0), -1);
            }
        }
